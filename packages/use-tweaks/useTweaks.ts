...
export function useTweaks<T extends Schema>(
    ...
    const _rootKey =
        typeof nameOrSchema === 'string' ? 'root.' + nameOrSchema : 'root';
    const _settings = useRef(
        typeof nameOrSchema === 'string'
            ? settings
            : (schemaOrSettings as Settings)
    );
    const _schema = useRef(
        typeof nameOrSchema === 'string'
            ? (schemaOrSettings as T)
            : nameOrSchema
    );
    ...
    useLayoutEffect(() => {
        ROOTPANE =
            ...
            new Pane({
                ..._settings.current,
                container: _settings.current?.container?.current!,
            });
        ...
        const disposablePanes = buildPane(
            _schema.current,
            _rootKey,
            setValue,
            _pane
