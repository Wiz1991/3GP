...
function transformSettings(settings: InputParams) {
    ...
    if (Array.isArray(settings.options)) {
        settings.options = settings.options.reduce(
            (acc: any, option: any) => ({ ...acc, [option]: option }),
            {}
        );
    }
    return settings;
...
export function getData(schema: Schema, rootPath: string) {
    const data: Record<string, unknown> = Object.entries(schema).reduce(
        (accValues, [key, input]) => {
            ...
            if (typeof input === 'object') {
                ...
                // DATA object, and if it isn't set, we set it to the schema value
                    ...
                    INPUTS[key] =
                        INPUTS[key] ?? (input as InputConstructor).value;
                } else {
            ...
            if ('type' in input) {
                if (input.type === SpecialInputTypes.MONITOR) {
                    ...
                    if (typeof ref === 'function') {
                        ...
                        monitor = rootPane
                            .addMonitor(myObj, 'current', {
                                label: title,
                                ...settings,
                            })
                            .on('update', updateFn);
                    } else if ('current' in ref) {
                        monitor = rootPane.addMonitor(ref, 'current', {
                            label: title,
                            ...settings,
                        });
                    } else {
                    ...
                    const folderPane = rootPane.addFolder({
                        title,
                        ...settings,
                    });
                ...
                const pane = rootPane
                    ...
                    .on('change', (ev: any) => setValue(key, ev.value));
